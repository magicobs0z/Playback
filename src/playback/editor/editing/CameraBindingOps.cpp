...
std::string addFreeCamera(model::EditorStateExt& state, const std::string& name) { model::CameraEntity camera; camera.id = makeId(state, "camera_"); camera.name = name.empty() ? "Camera " + std::to_string(state.cameras.size() + 1) : name; model::CameraKeyframe key; key.id = camera.id + ".key." + std::to_string(state.currentTick); key.tick = std::clamp(state.currentTick, 0, state.totalTicks); camera.keys.push_back(std::move(key)); state.cameras.push_back(camera); return camera.id; }
